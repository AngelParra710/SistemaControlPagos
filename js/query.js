    ...
    $("#reportesdeeys").click(function(){
      $.ajax({
        ...
        success: function (response) {
          ...
          gatos.forEach(({Fecha, tiposervicio, montodepago}) => {
              template +=`
              <tr>
              <td>${Fecha}</td>+
              <td>${tiposervicio}</td>+
              ...
              `
              sumadeentradas=sumadeentradas+parseInt(montodepago);
          })
          $("#bodyentradas").html(template);
          $(".totaldepagoentrada").text("$"+sumadeentradas);
          document.getElementById("montoCardE").innerHTML = `${sumadeentradas}`;
          llenatTGastos();
        }
      });
    })
...
/***************FUNCTIONS*************************** */
    ...
    const llenatTGastos = () => {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          document.getElementById("montoCardS").innerHTML = `${sumadesalidas}`;
          getTotales();
        }
      });
    }
    ...
    const getTotales = () => {
       var num1 = $('#montoCardE').text();
       var num2 = $('#montoCardS').text();
       console.log(num1 + " " + num2 )
       let result = num1 - num2;
       document.getElementById('montoCardT').innerHTML = result;
    }
});